...
const ENTITY_CASES=[
  ...
  {name:"PatternCaution",factory:TD.createPatternCaution,input:{user_id:"u1",movement_pattern:"HORIZONTAL_PUSH",level:"CAUTION_LOW",source_reason_code:"USER_REPORTED",created_at:"2026-08-15T00:00:00Z",review_at:"2026-09-15T00:00:00Z"},expectedKeys:["user_id","movement_pattern","level","source_reason_code","created_at","review_at"]},
...
console.log("========== Kanonische Enums/Registry-IDs (Korrektur nach STEP-01-Review) ==========");
{
  ...
  assertThrows(()=>TD.createSlotFunction({movement_pattern:"HORIZONTAL_PUSH",role:"PRIMARY",rep_character:"MODERATE",primary_muscle_bands:["NOT_A_MUSCLE"]}),"SlotFunction.primary_muscle_bands als blosse Muskel-ID-Strings (statt {canonical_volume_muscle_id,contribution_band}-Objekten) wird abgelehnt");
  assertThrows(()=>TD.createSlotFunction({movement_pattern:"HORIZONTAL_PUSH",role:"PRIMARY",rep_character:"MODERATE",primary_muscle_bands:[{canonical_volume_muscle_id:"NOT_A_MUSCLE",contribution_band:"PRIMARY_HIGH"}]}),"SlotFunction.primary_muscle_bands mit ungueltiger Muskel-ID wird abgelehnt");
