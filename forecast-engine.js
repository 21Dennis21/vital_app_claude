...
/* ===== 6. Juengste gueltige Bilanztage (nur laufender Kalendermonat) ===== */
function getRecentValidBalanceDays(dailyData, now, monthlySettings, maxDays){
  ...
  if(monthlyTdee==null) return [];
  const out=[];
  const cursor = new Date(y,m,now.getDate());
  const monthStart = new Date(y,m,1);
  while(cursor>=monthStart && out.length<maxDays){
...
function calculateCurrentDataQuality({recentDays, anchorAgeDays, calibrationProfile, now}){
  ...
  const coverage = recentDays.length / Math.max(1, Math.min(daysElapsedInMonth, FORECAST_CONFIG.maxRecentValidDays));
